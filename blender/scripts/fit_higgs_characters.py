"""Put the Higgsfield (Tripo H3.1) tennis players onto the V4 tennis rig.

The rig cannot change: the racket is animated in world space around the V4 arm lengths, so
moving a joint would pull the hand off the grip. The mesh is fitted to the rig instead:
  ...
  2. Build a temporary copy of the rig whose joints sit at the mesh's own joints (landmarks
     measured from horizontal slices, see check_character_anatomy.py) and skin the mesh to it.
...
The mesh's own fists are removed: the V4 grip hands hold the racket. The face decal is rebuilt
on the new head, inside the bare-skin window below the visor.
...
MODELS = ROOT / "SportsLibrary/ArtDirection/Tennis/Generated/models"
...
def save_textures(body, gender):
    """4K maps are far too heavy for the phone: ship 2K colour and 1K normal to Unity."""
    ...
    for node in mat.node_tree.nodes:
        ...
        img = node.image.copy()
        size = 2048 if kind == "Color" else 1024
        img.scale(size, size)
        path = TEXTURES / f"Higgs{gender}_{kind}.png"
