...
namespace GolfArcade.Course
{
    ...
    /// A round for one or more golfers. At home the players alternate shots: the first tees off,
    /// then the second, then the first plays their second shot, and so on round (PassTurn);
    /// whoever has holed out (or, in a contest, hit their one shot) drops out of the order, and
    /// once everyone is done the round moves to the next hole with the first player up (Advance).
    /// Everyone on a hole gets the same wind, from the round's seed, so the same seed on two
    /// phones gives an online match the same conditions. Pure C#, tested without a scene.
    public sealed class Match
    {
        ...
        /// hasn't played this hole (holed out, or hit their contest shot) — P1, P2, P1, P2… —
        /// skipping anyone who has, and staying with the last one left. False once every local
        ...
        public bool PassTurn()
        {
            for (int k = 1; k <= Players.Count; k++)
            {
                ...
                if (Players[i].IsLocal && !Played(Players[i], HoleIndex)) { TurnIndex = i; return true; }
            }
            return false;
        }
